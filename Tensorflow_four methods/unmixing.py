...
""" 不加这个PlotWhileTraining函数不会影响深度学习代码的正常运行，它只是一个辅助性的回调函数，用于在训练过程中帮助用户更好地理解模型的训练情况。
如果不需要实时监测损失函数的变化或绘制端元成分图像，可以不使用这个回调函数。但是，如果需要这些功能，则需要编写自己的代码来实现。
因此，这个函数主要是为了方便用户调试和优化模型，但并不是必需的。"""
...
"""这段代码实现了基于顶点成分分析(VCA)方法的端元提取算法。该算法可以用于高光谱数据中提取出代表地物光谱特征的端元。
具体而言，该算法的输入是一个高光谱数据矩阵Y，其中每行代表一个像素点的光谱，每列代表一个波段。
另外，算法还需要两个参数：R表示需要提取的端元数量，verbose表示是否输出详细信息。如果verbose为True，则算法会输出SNR估计值和选择的投影方式。
算法实现的过程主要分为以下几步：
1.对输入的高光谱数据矩阵进行标准化处理，即将每个波段的数据都减去均值并除以标准差，得到零均值和单位方差的数据矩阵。
2.根据SNR的估计值，选择使用投影到p-1维子空间还是使用投影到R维子空间的方式进行端元提取。
3.如果选择使用投影到p-1维子空间的方式，则需要先计算出p-projection matrix，然后将数据投影到R-1维子空间中，并选择一维作为新的端元。
4.如果选择使用投影到R维子空间的方式，则直接将数据投影到R维子空间中，并选择一维作为新的端元。
5.使用VCA算法提取出指定数量的端元，得到端元矩阵Ae和对应的像素点索引indice。同时，还会返回数据矩阵在端元空间中的表示Yp。
其中，VCA算法的具体实现过程是：
1.初始化一个R×R的矩阵A，并将最后一列设为[1,0,...,0]。
2.随机生成一个R×1的向量w，然后计算出f=w-A(A^T*A)^(-1)A^Tw，其中^T表示矩阵的转置，^(-1)表示矩阵的逆。
3.对f进行归一化处理，得到一个R×1的向量f'。
4.计算出f和数据矩阵在端元空间中的表示y的内积，得到一个R×1的向量v。
5.选取v中绝对值最大的元素所在的行数，将该行对应的像素点作为新的端元，并将该行对应的y中的元素作为A的新列。
重复步骤2-5，直到提取出指定数量的端元为止。
最终，该算法返回提取得到的端元矩阵Ae、对应的像素点索引indice和数据矩阵在端元空间中的表示Yp。"""
