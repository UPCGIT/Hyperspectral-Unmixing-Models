...
def plotEndmembersAndGT(endmembers, endmembersGT, endmember_path):
    num_endmembers = endmembers.shape[0]
...
def plotEndmembersvae(endmembers, endmembersGT, endmember_path):
    num_endmembers = endmembersGT.shape[0]
    n = int(num_endmembers // 2)
    if num_endmembers % 2 != 0:
        n = n + 1
    a = np.array([1, 1, 1])
    b = np.array(num_endmembers)
    for j in range(endmembers.shape[0]):
        SAD_index, SAD_endmember = order_endmembers(endmembersGT, endmembers[j].squeeze())
        if SAD_endmember.mean() < a.mean():
            a = SAD_endmember
            b = SAD_index
            c = endmembers[j]
    SAD_endmember = a
...
def plotAbundancesSimple(abundances, abundanceGT, abundance_path):
